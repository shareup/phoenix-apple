...
public final class OldSocket<WS: WebSocketProtocol> {
...
extension OldSocket {
    private func handle(message: IncomingMessage) {
        sync {
            switch message.event {
            case .join:
                guard let joinRef = message.joinRef else { break }
                handleJoin(topic: message.topic, joinRef: joinRef)
            case .leave:
                guard let joinRef = message.joinRef else {
                    break
                }
            ...
            case .reply:
                ...
                guard let status = message.payload["status"] as? String,
                    let response = message.payload["response"] as? Payload,
                    let originalMessage = _pushTracker.find(related: message) else {
                    handleCustom(message, joinRef: joinRef)
                    break
                }
                
                let reply = Reply(incomingMessage: message, status: status, response: response)
            ...
            _pushTracker.cleanup(related: message)
        }
    }
...
//        }
    ...
    private func handleCustom(_ message: IncomingMessage, joinRef: Ref) {
//        sync {
