...
export class Socket {
  ...
  leaveOpenTopic(topic) {
    let dupChannel = this.channels.find((c) =>
      c.topic === topic && (c.isJoined() || c.isJoining())
    );
    if (dupChannel) {
      if (this.hasLogger()) {
  ...
  static syncState(currentState, newState, onJoin, onLeave) {
    ...
    this.map(newState, (key, newPresence) => {
      ...
      if (currentPresence) {
        ...
        let joinedMetas = newPresence.metas.filter((m) =>
          curRefs.indexOf(m.phx_ref) < 0
        );
        let leftMetas = currentPresence.metas.filter((m) =>
  ...
  static syncDiff(currentState, { joins, leaves }, onJoin, onLeave) {
    ...
    this.map(joins, (key, newPresence) => {
      ...
      if (currentPresence) {
        ...
        let curMetas = currentPresence.metas.filter((m) =>
          joinedRefs.indexOf(m.phx_ref) < 0
        );
